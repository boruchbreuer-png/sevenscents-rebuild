# M-00 — Generation Record
**Status: ✅ CANONIZED 2026-07-07.**

> **THE CANON MASTER — M-00 = Candidate D**
> **Job ID `3cff50ce-c8cb-43f3-8452-7681ea49f028` · seed `511285`** · seedream_v4_5 · high · 16:9 · 5120×2880 · Prompt v2 · reference: Candidate B.
> Owner's verdict: strongest single-source light; best sense the room existed before the brand; most usable continuity geometry for relighting; most integrated 1931 lintel; strongest RITUAL read even before the loaf.
> **Every downstream asset references this job ID.** Do not reopen M-00 unless explicitly directed.
> **Candidate C** (`e7f4c2e0-1bb0-4642-b98a-b3793d6b2ab8`, seed `30100`) is preserved as the **rejected-but-strong alternate**; not canon, never referenced by downstream assets.

Historical record below. Pixels were not inspectable in-session in either round (CDN egress 403 by org policy; the Higgsfield MCP server does not support resource reads) — no scorecard was fabricated in either round. Visual review was the owner's, externally via the Higgsfield widget.

## Round 1 verdict (owner, external review)
- **Candidate B wins** — stronger single-source light, room geometry, 1931 presence, calmer composition, better negative space, no visible person.
- **Candidate A rejected** — visible human hand/body presence violates the brief.
- M-00 **not yet approved**; Round 2 ordered with Candidate B as base reference.

## Round 2 — exact metadata, do not alter

Both: model `seedream_v4_5` · quality `high` · aspect `16:9` · 5120×2880 · reference image: Candidate B (`489e7acb-9540-4872-a37c-c3b1b5935b61`, passed as `medias[{type:"job", role:"reference"}]`) · Prompt v2 = owner's Round-2 revisions verbatim (no people reinforced ×7 phrasings; tally wall de-ledgered to aged hand-made groups of five + one isolated fresh graphite mark; rustic/farmhouse/Pinterest negatives; groove "functional and earned, not theatrical"; jar label strata deepened; upper negative space reserved for relighting/overlays; 1924/2094 timelessness) · generated 2026-07-07 ≈18:53 UTC · cost: 1 credit for the round (2 credits total, both rounds).

### Candidate C
- **Job ID:** `e7f4c2e0-1bb0-4642-b98a-b3793d6b2ab8`
- **Seed:** `30100`
- Raw: `https://d8j0ntlcm91z4.cloudfront.net/user_3G8hyajw44R3O5Zf5BSpX1z91cI/hf_20260707_185330_e7f4c2e0-1bb0-4642-b98a-b3793d6b2ab8.png`

### Candidate D
- **Job ID:** `3cff50ce-c8cb-43f3-8452-7681ea49f028`
- **Seed:** `511285`
- Raw: `https://d8j0ntlcm91z4.cloudfront.net/user_3G8hyajw44R3O5Zf5BSpX1z91cI/hf_20260707_185330_3cff50ce-c8cb-43f3-8452-7681ea49f028.png`

## Owner's Round-2 judging standard (verbatim commitments)
Reads as RITUAL ignoring the loaf · room older than brand · one dominant light source · zero café/rustic/Pinterest cliché · geometry relightable across hours · canon present: 1931 lintel, aging tally groups of five, worn cutting groove, Mother jar label strata, single deep window beam, one fresh graphite mark, earned patina, quiet negative space.

## Awaiting from the owner
Winner (C or D or neither) · M-00 approval or rejection. If neither passes, the round allowance is exhausted — any further generation requires a new explicit allowance from the owner.

## Candidates — exact metadata, do not alter

Both: model `seedream_v4_5` · quality `high` · aspect `16:9` · 5120×2880 px · batch of 2, one job each · generated 2026-07-07 ≈18:32 UTC · prompt = **Prompt v1 verbatim** from `m00-brief.md` (submitted unmodified; the only edit from the committed draft was `café` → `cafe` for transport safety) · cost: 1 credit total for the round.

### Candidate A
- **Job ID:** `26718434-0a69-4540-b46a-b2d3950eb668`
- **Seed:** `292689`
- Raw: `https://d8j0ntlcm91z4.cloudfront.net/user_3G8hyajw44R3O5Zf5BSpX1z91cI/hf_20260707_183219_26718434-0a69-4540-b46a-b2d3950eb668.png`
- Preview: same path, `_min.webp`

### Candidate B
- **Job ID:** `489e7acb-9540-4872-a37c-c3b1b5935b61`
- **Seed:** `940708`
- Raw: `https://d8j0ntlcm91z4.cloudfront.net/user_3G8hyajw44R3O5Zf5BSpX1z91cI/hf_20260707_183219_489e7acb-9540-4872-a37c-c3b1b5935b61.png`
- Preview: same path, `_min.webp`

To re-display either in the widget: `job_display` with the job ID. To reference either in a future generation: pass the job ID as `medias[].value`.

## Awaiting from the owner
1. Which candidate is stronger.
2. Whether a second generation round is needed.
3. Any exact prompt revisions.
4. Approval or rejection of M-00.

## Round-2 allowance remaining (per the committed protocol)
One round of 2 candidates maximum, only if the owner orders it, using the owner's exact prompt revisions. Judging rubric stays as written in `m00-brief.md`.
